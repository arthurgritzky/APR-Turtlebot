...
#define PORT 9997			//Bei Turtlebot: Port richtig einstellen
...
int main(int argc, char const* argv[])
{
	...
	sem_t *semaphore_scan_write=sem_open(semaphore_name_scan_producer, O_CREAT, 0666, 0);		//"0"		//O_Create anstatt IPC_CREAT
	...
	sem_t *semaphore_scan_read=sem_open(semaphore_name_scan_consumer, O_CREAT, 0666, 1);	//"1"
    ...
    int shm_fd = shm_open(shared_memory_name_scan, O_CREAT | O_RDWR, 0666);			//Create SHM da "0_Create"
	...
	//extract the relevant data from the whole Turtlebot-Message
	extrahierteDaten=DataExtract(msg_String);

    const char* message = extrahierteDaten.c_str();
	
	//cout<<msg_String<<endl;

	//copy the extractred Data to the shared Memory
    strncpy(sharedData_scan->message, message, sizeof(sharedData_scan->message) - 1);	

    // Post to the semaphore that the writing-process is finished
	close(client_id_scan);
    sem_post(semaphore_scan_write);	//setzen Semaphre von 0 auf 1
...
string getMessage_TCP_IP_Connection()
{
	...
	while(true)
	{
	valread = read(client_id_scan, buffer, buffersize - 1); // subtract 1 for the null terminator at the end
